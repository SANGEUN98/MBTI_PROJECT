...
def get_request_url():
    url = 'http://www.kobis.or.kr/kobisopenapi/webservice/rest/code/searchCodeList.json'
    params = {'key': access_key, 'comCode' : '2201'
              }
...
def get_parsed_json(raw_json) :
    ...
    for record in raw_json['codes']:
        all_data.append(
            {"fullCd": record.get("fullCd") ,
             # {"baseDate": record["baseDate"],
             "korNm": record.get("korNm"),
            "engNm": record.get("engNm")}
        )
    return all_data
...
file_name = f'movieTypeCd_코드_{base_date}_{day_time}.json'
